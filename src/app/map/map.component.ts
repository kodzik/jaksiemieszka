...
export class MapComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    this.map.on('click', (e: { latlng: { lng: number; lat: number } }) => {
      ...
      if (this.markerService.enableMarkers) {
        this.onAddMarker = this.markerService.addMarker(
          this.map,
          e.latlng.lat,
          e.latlng.lng,
          this.markerService.enableMarkers
        );
    ...
    this.subShowDistricts = this.fabService.toggleDistricts.subscribe(
      (toggle) => {
  ...
  showDistricts(toggle: boolean): void {
